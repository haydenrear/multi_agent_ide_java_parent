...
class ActionRetryListenerImplTest {
    ...
    class Classification {
        ...
        void secondCompaction_statusIsMultiple() {
            ...
            history.addError(new ErrorDescriptor.CompactionError(
                    actionName, sessionKey, ErrorDescriptor.CompactionStatus.FIRST, false, "",null, contextId, EC));
            // Second compaction
            var result = listener.classify(
    ...
    class BlackboardHistoryErrorMethods {
        ...
        void addError_andErrorType_returnsLatest() {
            ...
            history.addError(new ErrorDescriptor.ParseError("action1", sessionKey, "bad json", "", null, key, EC));
            assertThat(history.errorType()).isInstanceOf(ErrorDescriptor.ParseError.class);

            history.addError(new ErrorDescriptor.TimeoutError("action2", sessionKey, 1, "", null, key, EC));
        ...
        void compactionStatus_tracksCompactionErrors() {
            ...
            history.addError(new ErrorDescriptor.CompactionError(
                    "action1", sessionKey, ErrorDescriptor.CompactionStatus.FIRST, false, "", null, key, EC));
            assertThat(history.compactionStatus()).isEqualTo(ErrorDescriptor.CompactionStatus.FIRST);

            history.addError(new ErrorDescriptor.CompactionError(
                    "action1", sessionKey, ErrorDescriptor.CompactionStatus.MULTIPLE, false, "", null, key, EC));
        ...
        void errorCountForRetrySequence_spansEntireRetryChain() {
            ...
            history.addError(new ErrorDescriptor.ParseError("old", sessionKey, "msg", "", null, key, EC));

            // First attempt (unmatched — no complete)
            history.addEntry("test", startEvent("action1", sessionKey, "node-1", "req-1"));
            history.addError(new ErrorDescriptor.TimeoutError("a", sessionKey, 1, "", null, key, EC));

            // Second attempt (also unmatched — retry chain continues)
            history.addEntry("test", startEvent("action1", sessionKey, "node-2", "req-2"));
            history.addError(new ErrorDescriptor.TimeoutError("a", sessionKey, 2, "", null, key, EC));
        ...
        void errorCount_countsByType() {
            ...
            history.addError(new ErrorDescriptor.ParseError("a", sessionKey, "msg", "", null, key, EC));
            history.addError(new ErrorDescriptor.ParseError("b", sessionKey, "msg2", "", null, key, EC));
            history.addError(new ErrorDescriptor.TimeoutError("c", sessionKey, 1, "", null, key, EC));
        ...
        void errorType_withSessionKey_returnsLastInRetrySequence() {
            ...
            history.addEntry("test", startEvent("action1", sessionKey, "node-1", "req-1"));
            history.addError(new ErrorDescriptor.ParseError("action1", sessionKey, "first", "", null, key, EC));
            history.addError(new ErrorDescriptor.TimeoutError("action1", sessionKey, 1, "", null, key, EC));
        ...
        void errorType_withSessionKey_excludesCompletedExecution() {
            ...
            history.addEntry("test", startEvent("action1", sessionKey, "node-1", "req-1"));
            history.addError(new ErrorDescriptor.ParseError("action1", sessionKey, "old error", "", null, key, EC));
            history.addEntry("test", completeEvent("action1", sessionKey, "node-1", "req-1", "node-1"));
        ...
        void errorType_withSessionKey_ignoresOtherSessions() {
            ...
            history.addError(new ErrorDescriptor.ParseError("action1", otherSession, "other session error", "", null, key, EC));
        ...
        void errorContext_accumulatesAcrossMultipleStartsAndErrors() {
            ...
            var error1 = new ErrorDescriptor.ParseError("action1", sessionKey, "bad json", "", null, key, EC);
            ...
            var error2 = new ErrorDescriptor.TimeoutError("action1", sessionKey, 1, "", null, key, ctx2);
            ...
            var error3 = new ErrorDescriptor.CompactionError("action1", sessionKey,
                    ErrorDescriptor.CompactionStatus.FIRST, false, "", null, key, ctx3);
        ...
        void errorContext_restartsAfterCompletedExecution() {
            ...
            history.addEntry("test", startEvent("action1", sessionKey, "node-1", "req-1"));
            history.addError(new ErrorDescriptor.ParseError("action1", sessionKey, "old", "", null, key, EC));
            history.addEntry("test", completeEvent("action1", sessionKey, "node-1", "req-1", "node-1"));
            ...
            var newError = new ErrorDescriptor.TimeoutError("action1", sessionKey, 1, "", null, key, EC);
