...
class AcpStreamWindowBufferTest {
    ...
    fun `flushes message window into generation when switching types`() {
        ...
        buffer.appendStreamWindow(
            "node-1",
            AcpStreamWindowBuffer.StreamWindowType.MESSAGE,
    ...
    fun `aggregates agent message chunks into single generation`() {
        ...
        buffer.appendStreamWindow(
            "node-1", AcpStreamWindowBuffer.StreamWindowType.MESSAGE, "Hel", ArtifactKey.createRoot(),
            ArtifactKey.createRoot()
        )
        buffer.appendStreamWindow(
            "node-1", AcpStreamWindowBuffer.StreamWindowType.MESSAGE, "lo", ArtifactKey.createRoot(),
            ArtifactKey.createRoot()
        )
    ...
    fun `aggregates thought chunks and emits on flush`() {
        ...
        buffer.appendStreamWindow(
            "node-1", AcpStreamWindowBuffer.StreamWindowType.THOUGHT, "think", ArtifactKey.createRoot(),
            ArtifactKey.createRoot()
        )
        buffer.appendStreamWindow(
            "node-1",
            AcpStreamWindowBuffer.StreamWindowType.THOUGHT,
            "ing",
            ArtifactKey.createRoot(),
            ArtifactKey.createRoot()
        )
    ...
    fun `publishes tool call events on flush`() {
        ...
        buffer.appendEventWindow(
            "node-1",
            AcpStreamWindowBuffer.StreamWindowType.TOOL_CALL,
            toolEvent,
            ArtifactKey.createRoot(),
            ArtifactKey.createRoot()
        )
    ...
    fun `publishes thought and user message chunks on flush`() {
        ...
        buffer.appendStreamWindow(
            "node-1",
            AcpStreamWindowBuffer.StreamWindowType.USER_MESSAGE,
            "User says hi",
            ArtifactKey.createRoot(),
            ArtifactKey.createRoot()
        )
