...
@Component
public class PropagatorSemanticValidator {

    public List<String> validate(PropagatorRegistrationRequest request) {
        List<String> errors = new ArrayList<>();
        if (request.name() == null || request.name().isBlank()) errors.add("Propagator name is required");
        if (request.description() == null || request.description().isBlank()) errors.add("Propagator description is required");
        if (request.sourcePath() == null || request.sourcePath().isBlank()) errors.add("Propagator sourcePath is required");
        ...
        if (request.layerBindings() != null) {
            for (int i = 0; i < request.layerBindings().size(); i++) {
                ...
                if (binding.matchOn() != null && !"ACTION_REQUEST".equals(binding.matchOn()) && !"ACTION_RESPONSE".equals(binding.matchOn())) {
                    errors.add(prefix + "matchOn must be ACTION_REQUEST or ACTION_RESPONSE");
                }
            }
        }
        return errors;
